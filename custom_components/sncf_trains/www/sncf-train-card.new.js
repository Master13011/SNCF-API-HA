...
class SncfTrainCard extends HTMLElement {
   ...
   * @returns {Promise<Array>} Un tableau d'entités de train avec des données fraîches, fusionnées de tous les devices et triées par date d'arrivée
   */
    ...
    return trains.map(train => {
      ...
      return `
        <div class="train-line">
          ...
          ${this.config.show_route_details && TA.stops_schedule ? this.renderTimeline(TA) : ''}
        </div>`;
    }).join('');
  }
  ...
  /**
   *
   * @param trainAttributes
   * @return {string}
   */
  renderTimeline(trainAttributes) {
    return `
      <div class="timeline-wrapper">
        <div class="timeline-line ${trainAttributes.hasDelay ? 'delayed-line' : ''}"></div>
  ...
  renderStops(stops) {
    return stops.map(stop => {
      ...
      const isStopDelayed = this.config.show_route_details && stop.amended_time && stop.base_time && (stop.amended_time !== stop.base_time);
      ...
      return `
        <div class="timeline-stop">
          <div class="timeline-dot ${isDeleted?'deleted-dot':(isAdded?'added-dot':(isStopDelayed?'delayed-dot':''))}"/>
    ...
    return `
      <div class="station">
        ...
        <div class="station-info">
          ...
          ${trainAttributes.delay_cause ? `<div class="delay-cause">${trainAttributes.delay_cause}</div>` : ''}
        </div>
      </div>
    `;
  }
